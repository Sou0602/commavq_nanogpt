...
def initialize_config():
    ...
    ## For Karpathy's nanogpt config override to work , these parameters need to initialized as globals.
...
def get_batch(split, config, train_data_in=None, val_data_in=None):
    """
    Get a batch of training or validation data.

    Args:
        split (str): Either 'train' or 'val' to specify the dataset split.
        config (model configuration): GPT Model Configs
        train_data_in (data,optional): For testing purpose,
        val_data_in (data,optional): For testing purpose,
    Returns:
...
def initialize_model(config, data_dir):
    ...
    if config["init_from"] == "scratch":
        ...
        if meta_vocab_size is None:
            print(
                "defaulting to vocab_size of 1048 (1024 rounded up for compiler compatibility)"
            )
...
# Training loop
def train_model(
    model, optimizer, scaler, config, iter_num, best_val_loss, master_process
):
    """
    ...
    X, Y = get_batch("train", config=config)  # fetch the very first batch
    t0 = time.time()
    ...
    while True:
        ...
        if iter_num % config["eval_interval"] == 0 and master_process:
            losses = estimate_loss(config=config)
            print(
                ...
                wandb.log(
                    {
                        ...
                        "val/loss": losses["val"],
                        "lr": lr,
                        "mfu": running_mfu * 100,  # convert to percentage for A100
                    }
                )
        ...
        for micro_step in range(config["gradient_accumulation_steps"]):
            ...
            X, Y = get_batch("train", config=config)
...
if __name__ == "__main__":
    master_process = True
    seed_offset = 0
    torch.manual_seed(1337 + seed_offset)  ## From Karpathy's nanogpt
